...
def english_correction(text):
    ...
    for curr_correction in reversed(corrections):
        corrected_text = corrected_text[:curr_correction["startIndex"]] + "__" + curr_correction['suggestion'] + "__" + corrected_text[curr_correction["endIndex"]:]
...
def correction_with_punctuation(text):
    ...
    for it in range(2):
        ...
        for mistake in grammar_mistakes:
            if len(mistake['better']) == 0:
                continue
            grammar_corrected_text += text[i:mistake['offset']] + '__' + mistake['better'][0] + '__'
    ...
    for mistake in spelling_mistakes:
        spelling_corrected_text += grammar_corrected_text[i:mistake['offset']] + '__' + mistake['better'][0] + '__'
